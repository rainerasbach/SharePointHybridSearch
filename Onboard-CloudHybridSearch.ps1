...
if ($ACS_APPPRINCIPALID -eq $null) {
    ...
    New-Variable -Option Constant -Name SCS_AUTHORITIES -Value @(
     ...
     #ObsoleteGermanCloud   "*.search.production.de.azuretrafficmanager.de",
...
function Configure-LocalSharePointFarm
{
    ...
    $acsProxy = Get-SPServiceApplicationProxy | ? {
        $_.GetType().FullName -eq "Microsoft.SharePoint.Administration.SPAzureAccessControlServiceApplicationProxy" -and
    ...
    # SPO proxy
    $spoProxy = Get-SPServiceApplicationProxy | ? {$_.GetType().FullName -eq "Microsoft.SharePoint.Administration.SPOnlineApplicationPrincipalManagementServiceApplicationProxy" -and $_.OnlineTenantUri -eq [System.Uri] $PortalUrl}
    if ($spoProxy -eq $null) {
...
function Prepare-Environment
{
	...
	if ($MSOnlinePsModuleInstalled -eq $null)
	{
        ...
        throw ("MSOnline AAD PowerShell module is not installed.")
	}
	else
	{
